...
func (s *connectionManager) removeLongInactiveClients() {
	...
	candidates := s.findLongInactiveClients()
	s.remove(candidates)
}

func (s *connectionManager) findLongInactiveClients() []int64 {
	candidates := make([]int64, 0)

	for candidate, info := range s.clientInfos {
		if time.Since(info.lastActiveTime) > s.ttl {
...
func (s *connectionManager) remove(candidates []int64) {
	for _, candidate := range candidates {
		cli, ok := s.clientInfos[candidate]
		if ok {
			cli.logDeregister()
