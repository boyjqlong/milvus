...
SegmentSealedImpl::LoadFieldData(const LoadFieldDataInfo& info) {
    ...
    if (SystemProperty::Instance().IsSystem(field_id)) {
        ...
        AssertInfo(data_type == engine::DataType(info.field_data->type()), "field type of load data is inconsistent with the schema");
        auto source_data = GetFieldData(field_meta, info.field_data);
        auto vec_data = insert_record_.get_field_data_base(field_id);
        AssertInfo(vec_data->empty(), "already exists");

        // write data under lock
        std::unique_lock lck(mutex_);

        // insert data to insertRecord
        vec_data->fill_chunk_data(source_data, size);
        ...
        if (field_meta.is_vector()) {
            ...
            index = query::generate_scalar_index(vec_data->get_span_base(0), data_type);
...
SegmentSealedImpl::vector_search(int64_t vec_count,
    ...
    results.seg_offsets_ = std::move(sub_qr.mutable_ids());
...
SegmentSealedImpl::bulk_subscript(SystemFieldType system_type,
    ...
    bulk_subscript_impl<int64_t>(field_data, seg_offsets, count, INVALID_ID, output);
}

template <typename T>
void
SegmentSealedImpl::bulk_subscript_impl(const void* src_raw, const int64_t* seg_offsets, int64_t count, T default_value, void* dst_raw) {
    static_assert(IsScalar<T>);
    auto src = reinterpret_cast<const T*>(src_raw);
    auto dst = reinterpret_cast<T*>(dst_raw);
    for (int64_t i = 0; i < count; ++i) {
        auto offset = seg_offsets[i];
        dst[i] = (offset == INVALID_SEG_OFFSET ? default_value : src[offset]);
    }
}
...
SegmentSealedImpl::bulk_subscript(FieldId field_id,
    ...
    switch (field_meta.get_data_type()) {
        case DataType::BOOL: {
            ...
            bulk_subscript_impl<bool>(src_vec, seg_offsets, count, false, output.data());
            return CreateScalarDataArrayFrom(output.data(), count, field_meta);
        }
        case DataType::INT8: {
            FixedVector<int8_t> output(count);
            bulk_subscript_impl<int8_t>(src_vec, seg_offsets, count, INVALID_ID, output.data());
            return CreateScalarDataArrayFrom(output.data(), count, field_meta);
        }
        case DataType::INT16: {
            FixedVector<int16_t> output(count);
            bulk_subscript_impl<int16_t>(src_vec, seg_offsets, count, INVALID_ID, output.data());
            return CreateScalarDataArrayFrom(output.data(), count, field_meta);
        }
        case DataType::INT32: {
            FixedVector<int32_t> output(count);
            bulk_subscript_impl<int32_t>(src_vec, seg_offsets, count, INVALID_ID, output.data());
            return CreateScalarDataArrayFrom(output.data(), count, field_meta);
        }
        case DataType::INT64: {
            FixedVector<int64_t> output(count);
            bulk_subscript_impl<int64_t>(src_vec, seg_offsets, count, INVALID_ID, output.data());
            return CreateScalarDataArrayFrom(output.data(), count, field_meta);
        }
        case DataType::FLOAT: {
            FixedVector<float> output(count);
            bulk_subscript_impl<float>(src_vec, seg_offsets, count, INVALID_ID, output.data());
            return CreateScalarDataArrayFrom(output.data(), count, field_meta);
        }
        case DataType::DOUBLE: {
            FixedVector<double> output(count);
            bulk_subscript_impl<double>(src_vec, seg_offsets, count, INVALID_ID, output.data());
            return CreateScalarDataArrayFrom(output.data(), count, field_meta);
        }
        case DataType::VarChar: {
            FixedVector<std::string> output(count);
            bulk_subscript_impl<std::string>(src_vec, seg_offsets, count, INVALID_STRING, output.data());
            return CreateScalarDataArrayFrom(output.data(), count, field_meta);
        }
