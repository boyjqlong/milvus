...
SegmentInternalInterface::FillPrimaryKeys(const query::Plan* plan, SearchResult& results) const {
    ...
    AssertInfo(IsPrimaryKeyDataType(get_schema()[pk_field_id].get_data_type()), "Primary key field is not INT64 or VarChar type");
    ...
    std::copy_n(pks.data(), size, results.primary_keys_.data());
}
...
SegmentInternalInterface::Retrieve(const query::RetrievePlan* plan, Timestamp timestamp) const {
    ...
    for (auto field_id : plan->field_ids_) {
        ...
        auto col = bulk_subscript(field_id, retrieve_results.result_offsets_.data(),
