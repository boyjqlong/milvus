...
SegmentGrowingImpl::Insert(int64_t reserved_offset,
    ...
    for (auto& field: schema_->get_fields()) {
        AssertInfo(field_id_to_offset.count(field.first), "Cannot find field_id");
    }

    // step 2: sort timestamp
    // sort timestamps when flow graph receive insert msgs
    // sort timestamps of internalPb.insertRequest in proxy
    // avoid repeat copy in c++
...
SegmentGrowingImpl::bulk_subscript(FieldId field_id,
    ...
    switch (field_meta.get_data_type()) {
        case DataType::BOOL: {
            ...
            bulk_subscript_impl<bool>(*vec_ptr, seg_offsets, count, false, output.data());
            return CreateScalarDataArrayFrom(output.data(), count, field_meta);
        }
        case DataType::INT8: {
            FixedVector<bool> output(count);
            bulk_subscript_impl<int8_t>(*vec_ptr, seg_offsets, count, INVALID_ID, output.data());
            return CreateScalarDataArrayFrom(output.data(), count, field_meta);
        }
        case DataType::INT16: {
            FixedVector<int16_t> output(count);
            bulk_subscript_impl<int16_t>(*vec_ptr, seg_offsets, count, INVALID_ID, output.data());
            return CreateScalarDataArrayFrom(output.data(), count, field_meta);
        }
        case DataType::INT32: {
            FixedVector<int32_t> output(count);
            bulk_subscript_impl<int32_t>(*vec_ptr, seg_offsets, count, INVALID_ID, output.data());
            return CreateScalarDataArrayFrom(output.data(), count, field_meta);
        }
        case DataType::INT64: {
            FixedVector<int64_t> output(count);
            bulk_subscript_impl<int64_t>(*vec_ptr, seg_offsets, count, INVALID_ID, output.data());
            return CreateScalarDataArrayFrom(output.data(), count, field_meta);
        }
        case DataType::FLOAT: {
            FixedVector<float> output(count);
            bulk_subscript_impl<float>(*vec_ptr, seg_offsets, count, INVALID_ID, output.data());
            return CreateScalarDataArrayFrom(output.data(), count, field_meta);
        }
        case DataType::DOUBLE: {
            FixedVector<double> output(count);
            bulk_subscript_impl<double>(*vec_ptr, seg_offsets, count, INVALID_ID, output.data());
            return CreateScalarDataArrayFrom(output.data(), count, field_meta);
        }
        case DataType::VarChar: {
            FixedVector<std::string> output(count);
            bulk_subscript_impl<std::string>(*vec_ptr, seg_offsets, count, INVALID_STRING, output.data());
            return CreateScalarDataArrayFrom(output.data(), count, field_meta);
        }
        default: {
...
SegmentGrowingImpl::bulk_subscript(SystemFieldType system_type,
    ...
    switch (system_type) {
        ...
        case SystemFieldType::RowId:
            bulk_subscript_impl<int64_t>(this->insert_record_.row_ids_, seg_offsets, count, INVALID_ID, output);
            break;
        default:
